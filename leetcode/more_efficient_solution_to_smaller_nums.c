...
which are less than the current number using the get_sum() function, 
...
int* count_smaller(int* nums, int nums_size, int* return_size) {
    *return_size = nums_size;
    int *counts = (int *)malloc(nums_size * sizeof(int));
    BIT *bit = createBIT();
    for (int i = nums_size - 1; i >= 0; i--) {
        counts[i] = get_sum(bit, nums[i] + OFFSET);
